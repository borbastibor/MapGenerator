function openSimplexNoise(clientSeed) {
	...
	const API = {
		...
		sum(method, point, frequency, amplitude, octaves, lacunarity, presistence, range) {
			let sum;

			switch (method) {
				case 'noise2D':
					sum = this.noise2D(point.x * frequency, point.y * frequency);
					break;
			
				case 'noise3D':
					sum = this.noise3D(point.x * frequency, point.y * frequency, point.z * frequency);
					break;
				
				case 'noise4D':
					sum = this.noise4D(point.x * frequency, point.y * frequency, point.z * frequency, point.w * frequency);
					break;
			}
			...
			for (let i = 0; i < octaves; i++) {
				...
				switch (method) {
					case 'noise2D':
						sec_sum = this.noise2D(point.x * frequency, point.y * frequency) * amplitude;
						break;
				
					case 'noise3D':
						sec_sum = this.noise3D(point.x * frequency, point.y * frequency, point.z * frequency) * amplitude;
						break;
					
					case 'noise4D':
						sec_sum = this.noise4D(point.x * frequency, point.y * frequency, point.z * frequency, point.w * frequency) * amplitude;
						break;
				}
